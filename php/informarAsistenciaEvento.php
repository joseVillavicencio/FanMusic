<?php
	...
	if($conexion->connect_errno ) {
		...
		$sql = "call  obtenerIDEvento('".$nombreEv."');";	
		if($result = $conexion->query($sql)){
			if($result->num_rows >0){
				while($fila=mysqli_fetch_row($result)){
					$idEvento=$fila[0] ; /**Hasta acá llega bien los parametros :)**/
					$conexion=conectar();
					$sql2 = "call revisarRespuesta('".$id."','".$idEvento."');";
						...
						if($result2->num_rows >0){
							while($fila=mysqli_fetch_row($result2)){
								...
								if($respuesta == 1){
									...
									if($result21 = $conexion->query($sql21)){ /**Hasta acá actualiza bien :)**/
										$conexion=conectar();
										$sql3 = "call obtenerCantUno('".$fechaEscogida."');";
										if($result3 = $conexion->query($sql3)){
											if($result3->num_rows >0){
												while($fila=mysqli_fetch_row($result3))	$cont=$fila[0] ;
												$conexion=conectar();
										...
										if($result4 = $conexion->query($sql4)){
											if($result4->num_rows >0){
												while($fila=mysqli_fetch_row($result4))	$cont=$fila[0] ;
												$conexion=conectar();
										...
										if($result5= $conexion->query($sql5)){
											if($result5->num_rows >0){
												echo "entro";
												while($fila=mysqli_fetch_row($result5))	$cont=$fila[0] ;
												$conexion=conectar();
