<?php
	...
	$titulo=$_POST["titulo"]; 
	if($titulo==""){
		echo "Debe agregar un título.<button onclick='location.href=".'"'."//localhost/FanMusic/ventanaPopFinanzaG.php".'"'."'>Volver</button>";
	}else{
		...
		if($tipo!="jpeg"){
			if($tipo!="jpg"){
				if($tipo!="gif"){
					...
					echo "Ha Ocurrido un problema, la imagen seleccionada no es del formato soportado por el sistema, favor intentar con JPEG, JPG y GIF.<br><button onclick='location.href=".'"'."//localhost/FanMusic/ventanaPopFinanzaG.php".'"'."'>Volver</button>";
					$tipocorrect=false;
				}
			}
		}
